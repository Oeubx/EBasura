...
class AEBasuraSupabaseUser extends BaseAuthUser {
  AEBasuraSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> aEBasuraSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AEBasuraSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
